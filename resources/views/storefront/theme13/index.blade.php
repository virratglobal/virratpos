@extends('storefront.layout.theme13')
@section('page-title')
    {{__('Home')}}
@endsection
@push('css-page')
...
<div class="wrapper">
    ...
    <section class="main-home-first-section">
        <div class="offset-container offset-left">
            <div class="row justify-content-end">
                @if($theme13_product != null)
                    <div class="col-lg-6 col-12">
                        <div class="home-banner-product padding-top">
                            <div class="row align-items-center">
                                <div class="col-md-6 col-12">
                                    <div class="banner-title">
                                        <h1>{{$theme13_product->name}}</h1>
                                    </div>
                                    <div class="banner-links">
                                        <ul class="banner-list">
                                            <li><a href="{{route('store.product.product_view',[$store->slug,$theme13_product->id])}}" class="text-dark">{{__('DESCRIPTION')}}</a></li>
                                            <li><a href="{{route('store.product.product_view',[$store->slug,$theme13_product->id])}}" class="text-dark">{{__('SPECIFICATION')}}</a></li>
                                            <li><a href="{{route('store.product.product_view',[$store->slug,$theme13_product->id])}}" class="text-dark">{{__('DETAILS')}}</a></li>
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-md-6 col-12">
                                    <div class="product-img">
                                        {{--  @if($theme13_product_image != null && $theme13_product_image->count()>0)
                                            <img src="{{$catimg .$theme13_product_image[0]['product_images']}}" alt="">
                                        @endif  --}}
                                        @if($theme13_product_image != null &&  $theme13_product_image->count()>0)
                                            <img src="{{$catimg.$theme13_product_image[0]['product_images']}}" alt="">
                                        @endif
                                    </div>
                                    @if($theme13_product_image != null )
                                        @if($theme13_product['enable_product_variant'] == 'on')
                                            <div class="price justify-content-center">
                                                <ins><span class="currency-type"></span> {{__('In variant')}}</ins>
                                            </div>
                                            <div class="cart-btns">
                                                <a href="{{route('store.product.product_view',[$store->slug,$theme13_product->id])}}" class="btn">{{__('ADD TO CART')}}<i class="fas fa-shopping-basket"></i></a>
                                                @if(!empty($wishlist) && isset($wishlist[$theme13_product->id]['product_id']))
                                                    @if($wishlist[$theme13_product->id]['product_id'] != $theme13_product->id)
                                                        <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product->id}}" data-id="{{$theme13_product->id}}"><i class="far fa-heart"></i></a>
                                                    @else
                                                        <a class="btn-secondary wish-btn"><i class="fas fa-heart" data-id="{{$theme13_product->id}}"></i></a>
                                                    @endif
                                                @else
                                                    <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product->id}}" data-id="{{$theme13_product->id}}"><i class="far fa-heart"></i></a>
                                                @endif
                                        ...
                                        @else
                                            <div class="price justify-content-center">
                                                <ins>{{\App\Models\Utility::priceFormat($theme13_product->price)}}</ins>
                                            </div>
                                            <div class="cart-btns">
                                                <a class="btn add_to_cart" data-id="{{$theme13_product->id}}">{{__('ADD TO CART')}}<i class="fas fa-shopping-basket"></i></a>
                                                @if(!empty($wishlist) && isset($wishlist[$theme13_product->id]['product_id']))
                                                    @if($wishlist[$theme13_product->id]['product_id'] != $theme13_product->id)
                                                        <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product->id}}" data-id="{{$theme13_product->id}}"><i class="far fa-heart"></i></a>
                                                    @else
                        ...
                        @foreach($getStoreThemeSetting as $storethemesetting)
                            @if($storethemesetting['section_name'] == 'Banner-Image')
                                <div class="main-banner-img">
                                    <img src="{{$imgpath.(!empty($storethemesetting['inner-list'][0]['field_default_text'])?$storethemesetting['inner-list'][0]['field_default_text']:'theme13/header/header_img_3.png')}}" alt="">
                                </div>
                            @endif
                        @endforeach
                    @endif
    ...
    @endif
    @if($theme13_product_random != null && $theme13_product_random->count()>0)
        <section class="your-time-section padding-bottom ">
            <div class="container">
                <div class="row  align-items-center ">
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="rightside-img">
                            @if(!empty($theme13_product_random->is_cover))
                                <img src="{{$productImg.(!empty($theme13_product_random->is_cover)?$theme13_product_random->is_cover:'')}}" title="{{$theme13_product_random->name}}" alt="">
                            @else
                                <img src="{{asset(Storage::url('uploads/is_cover_image/default.jpg'))}}" alt="" title="{{$theme13_product_random->name}}">
                            @endif
                        </div>
                    </div>
                    <div class="col-lg-6 col-md-6 col-12">
                        <div class="product-desk">
                            <div class="title-inner">
                                <h2><a href="{{route('store.product.product_view',[$store->slug,$theme13_product_random->id])}}">{{$theme13_product_random->name}}</a></h2>
                            </div>
                            <p>{!! $theme13_product_random->detail !!}</p>
                            @if($theme13_product_random['enable_product_variant'] == 'on')
                                <div class="cart-btn align-items-center">
                                    <a href="{{route('store.product.product_view',[$store->slug,$theme13_product_random->id])}}" class="btn"><i class="fas fa-shopping-basket"></i></a>
                                    @if(Auth::guard('customers')->check())
                                        @if(!empty($wishlist) && isset($wishlist[$theme13_product_random->id]['product_id']))
                                            @if($wishlist[$theme13_product_random->id]['product_id'] != $theme13_product_random->id)
                                                <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product_random->id}}" data-id="{{$theme13_product_random->id}}"><i class="far fa-heart"></i></a>
                                            @else
                                                <a class="btn-secondary wish-btn"><i class="fas fa-heart" data-id="{{$theme13_product_random->id}}"></i></a>
                                            @endif
                                        @else
                                            <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product_random->id}}" data-id="{{$theme13_product_random->id}}"><i class="far fa-heart"></i></a>
                                        @endif
                            ...
                            @else
                                <div class="cart-btn align-items-center">
                                    <a class="btn add_to_cart" data-id="{{$theme13_product_random->id}}">{{__('Add to cart')}}<i class="fas fa-shopping-basket"></i></a>
                                    @if(Auth::guard('customers')->check())
                                        @if(!empty($wishlist) && isset($wishlist[$theme13_product_random->id]['product_id']))
                                            @if($wishlist[$theme13_product_random->id]['product_id'] != $theme13_product_random->id)
                                                <a class="btn-secondary wish-btn add_to_wishlist wishlist_{{$theme13_product_random->id}}" data-id="{{$theme13_product_random->id}}"><i class="far fa-heart"></i></a>
                                    ...
                                    @endif
                                    <div class="price">
                                        <ins>{{\App\Models\Utility::priceFormat($theme13_product_random->price)}}</ins>
                                    </div>
                                </div>
                            @endif
                        </div>
                                        ...
                                        {{--  <div class="testimonial-slider">  --}}
                                            <div class="testimonial-card">
                                                ...
                                                <div class="testimonial-img">
                                                    <img src="{{ $imgpath. (!empty($storethemesetting['homepage-testimonial-card-image'][$i]['field_prev_text']) ? $storethemesetting['homepage-testimonial-card-image'][$i]['field_prev_text'] : 'theme13/header/header_img_3.png') }}" alt="">
                                                </div>
                                            </div>
                                        {{--  </div>  --}}
                                    @endif
                            ...
                            @else
                                @for ($i = 0; $i < $storethemesetting['loop_number']; $i++)
                                        ...
                                        <div class="testimonial-card">
                                            ...
                                            <div class="testimonial-img">
                                                <img src="{{$imgpath.(!empty($storethemesetting['inner-list'][1]['field_default_text']) ? $storethemesetting['inner-list'][1]['field_default_text'] : 'theme13/header/header_img_3.png')}}" alt="">
                                            </div>
                                        </div>
